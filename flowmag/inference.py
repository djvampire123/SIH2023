...
def overlay_videos(frames1, frames2, save_path):
    ...
    saved_frames = [(255 * img.squeeze().permute(1, 2, 0).flip([-1]).numpy()).astype(np.uint8) for img in overlaid_frames]
    write_video(saved_frames, 30, save_path)
    print(f'Saved the overlaid video to {save_path}')
...
if __name__ == '__main__':
    ...
    if len(processed_videos) > 1:
        overlay_save_path = str(save_dir / 'overlayed_video.mp4')
        overlay_videos(processed_videos[0], processed_videos[1], overlay_save_path)
